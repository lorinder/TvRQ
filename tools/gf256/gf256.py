...
assert len(OCT_EXP) == 510, "OCT_EXP table has wrong size."

OCT_LOG = [
0xff, # <-- Filler entry for 1-based indexing
...
assert len(OCT_LOG) == 255 + 1, "OCT_LOG table has wrong size."

#
# primitive arithmetic operations, using ints.
#
def add(a, b):
	return a^b
    ...
    for i in range(len(A)):
        ...
        for j in range(len(B)):
            ...
            for k in range(len(A[0])):
                x = add(x, mul(A[i][k], B[k][j]))
            v.append(x)
        R.append(v)
    return R

def hexrepr(A):
    return [ ["%x" % x for x in r] for r in R ]

#
# gf256 class for convenient command line calculations
#
class El(object):
    def __init__(self, val):
        self.val = val
    def __add__(self, other):
        return El(add(self.val, other.val))
    def __sub__(self, other):
        return self.__add__(other)
    def __mul__(self, other):
        return El(mul(self.val, other.val))
    def __truediv__(self, other):
        return El(mul(self.val, inv(other.val)))
    def __repr__(self):
        return "El(0x%02x)" % (self.val,)
